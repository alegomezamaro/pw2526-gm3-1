...
public class FamiliaApiController {
    ...
    public ResponseEntity<?> desvincularSocio(@PathVariable Integer id, @RequestBody Map<String, String> body) {
        ...
        Inscripcion ins = inscripcionRepository.findInscripcionByDNI(socio.getDni());

        if(ins != null){
            boolean ok = inscripcionRepository.deleteInscripcionByDniTitular(socio.getDni());
            if(!ok){
                return ResponseEntity.badRequest().body("No se pudo borrar la inscripcion");
            }
        }
